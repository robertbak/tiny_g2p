...
| `../tiny-g2p/` | the sibling repo: the same model as a static binary, a `.wasm` and ONNX |
...
it ports cleanly to anything, and that port is its own repository. The sibling
[`tiny-g2p`](https://github.com/robertbak/tiny-g2p) carries the same weights as
a **static binary**, a **`.wasm`** module and an **ONNX** graph; all three are
bit-exact against the Python model on every one of the 6,725 golden words, and
ONNX agrees with torch on every argmax.

```bash
make export                              # writes the blob and the ONNX into ../tiny-g2p
make -C ../tiny-g2p test                 # the Rust tests, parity included
../tiny-g2p/target/release/tinyg2p predict "Szczebrzeszyn bmw"
# ʂ tʂ ɛ b ʐ ɛ ʂ ɨ n̪
# b ɛ m ɛ v u
```
...
The blob this repository exports is what that one embeds, so the export has to
run before the Rust crate can build at all. That is the whole coupling between
the two: `RUST_DIR` names the checkout, and nothing else crosses.

### The exception path lives there too

Words the model should not own are looked up rather than guessed, and the
dictionaries that do it are generated by the Rust CLI, so they live with it
alongside a copy of the gold split:

```bash
make -C ../tiny-g2p miss-lexicon         # regenerates data/base.dict and data/seed.dict
```

`base.dict` takes the held-out split from 98.18% to 99.40% measured, and from
98.96% to 99.97% gold-corrected, for 56 KB. The alternative — loading MFA's
whole dictionary — costs 7 MB and about 85 MB resident, and overrides the model
on all 134,507 words including the 45 that MFA is known to be wrong about.

Both files are *suggested* readings, loaded with `--suggest`, and they answer
only where nothing more authoritative has an answer. A word somebody asserts --
their own name, with `--lexicon` -- outranks them: a dictionary's reading is
evidence, and a person saying how their name is said is not evidence but fact.

The examples of all this on real speech are in that repository's `examples/`:
eight BIGOS utterances with their phones, a walkthrough of what happened in
each, and one asserted name the model gets wrong on its own.

`../tiny-g2p/README.md` has the build commands, the parity table and the layout.
...
build over in `tiny-g2p`, so anyone shipping those is redistributing
